...
async function MarketInfoPage() {
  ...
  const yesterdayRankings = await marketService.getUsStockRanking(
    subDays(new Date(), 1)
  );
    ...
    <PageContainer>
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <RaiseCard stockRankings={stockRankings} />
          <LowerCard stockRankings={stockRankings} />
          <HotCard marketTradingValueTrends={returnValueInfo} />
        </div>

        {/* 시가총액 순위 테이블 */}
        <CapSortedTable stockRankings={stockRankings} />
