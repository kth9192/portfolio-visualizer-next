...
export const getBacktestingData= async({ticker,startDate,endDate,rebalanceFrequency}:BacktestingReq):Promise<AxiosResponse<ApiResponse<BacktestingRes>>> => {
    ...
    const response = await apiInstance.get("/backtesting",{
        params:{
            ticker:ticker.join(","),
